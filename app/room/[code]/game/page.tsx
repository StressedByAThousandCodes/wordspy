...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    if (round.phase === 'result') {
      // Result timer expired — check if server already returned us to lobby.
      // If not, call advance directly which now handles result phase too.
      setTimeout(async () => {
        ...
        if (roomData?.status === 'lobby') {
          ...
          // Server setTimeout may have failed — call advance as fallback
  ...
  const phaseDuration = round.created_at
    ? Math.round((new Date(round.phase_ends_at).getTime() - new Date(round.created_at).getTime()) / 1000)
    : 30
        ...
        <Card className="p-5 space-y-4">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm" style={{ color: 'var(--text-3)' }}>
              Civilians had{' '}
              <span style={{ color: 'var(--accent)', fontWeight: 600 }}>{round.civilian_word}</span>
              {' · '}
              Spies had{' '}
              <span style={{ color: 'var(--danger)', fontWeight: 600 }}>{round.spy_word}</span>
            </p>
          </div>
