...
export default function LobbyPage() {
  ...
  const fetchPlayers = useCallback(
    async (roomId: string): Promise<Player[]> => {
      ...
      try {
        sessionStorage.setItem(`lobby_players_${roomId}`, JSON.stringify(rows));
      } catch {
        /* quota */
      }
  ...
  const fetchPlayersWithRetry = useCallback(
    async (roomId: string): Promise<Player[]> => {
      ...
      try {
        ...
        if (cached) {
          const cachedRows = JSON.parse(cached) as Player[];
          if (cachedRows.length > 0) {
            setPlayers(cachedRows);
            prevPlayerIds.current = new Set(cachedRows.map((p) => p.id));
          }
        }
      } catch {
  ...
  return (
    ...
    >
      ...
      {showQR && (
        <div
          className="border-b py-4 animate-fadeIn"
      ...
      <div className="flex-1 flex overflow-hidden max-w-7xl mx-auto w-full">
        ...
        <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="space-y-2">
              ...
              {players.length < minPlayers && (
                <p className="text-xs" style={{ color: "var(--text-3)" }}>
                  ...
                  {minPlayers - players.length !== 1 ? "s" : ""} to start
                </p>
              )}
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                {players.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl bg-gradient-to-b ${color.bg}`}
                      />
                      <div
                        className={`relative w-10 h-10 rounded-xl bg-gradient-to-br ${color.bg} flex items-center justify-center text-lg shrink-0`}
                      >
                        {emoji}
                        {p.is_ready && (
                          <div
                            className="absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 flex items-center justify-center text-[8px] font-bold text-white"
                            style={{
          ...
          {/* Mobile bottom bar */}
          <div
            className="lg:hidden fixed bottom-0 left-0 right-0 border-t px-4 pt-3 pb-6 z-20"
            style={{ background: "var(--bg)", borderColor: "var(--border)" }}
          >
            {error && (
              <p
                className="text-xs mb-2 px-3 py-2 rounded-xl"
                style={{
                  background: "var(--danger-bg)",
                  color: "var(--danger)",
                }}
            ...
            <div className="space-y-2">
              <Button
                variant={myPlayer?.is_ready ? "secondary" : "success"}
                size="lg"
                onClick={toggleReady}
              >
          ...
          {/* Desktop bottom bar */}
          <div
            className="hidden lg:block border-t px-6 py-3"
            style={{ background: "var(--bg)", borderColor: "var(--border)" }}
          >
            {error && (
              <p
      ...
      >
        ...
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              <div
                className={`w-6 h-6 rounded-lg bg-gradient-to-br ${color.bg} flex items-center justify-center text-xs shrink-0`}
              >
                {emoji}
              </div>
              <div
        ...
        <div className="flex border-b" style={{ borderColor: "var(--border)" }}>
          {tabs.map((tab) => (
            <button
              ...
              style={{
                color:
                  settingsTab === tab.id ? "var(--accent)" : "var(--text-3)",
                borderBottom:
                  settingsTab === tab.id
                    ? "2px solid var(--accent)"
                    : "2px solid transparent",
                background:
                  settingsTab === tab.id ? "var(--accent-bg)" : "transparent",
              }}
            >
              {tab.label}
            </button>
          ...
          {settingsTab === "game" && (
            <div className="space-y-4">
              <SettingCard
                icon="🕵️"
                label="Spies"
                description="Hidden players with a different word"
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button
                      key={p.label}
                      onClick={() => {
                        updateSetting("spy_count", p.spies);
                        updateSetting("describe_seconds", p.describe);
                        updateSetting("discuss_seconds", p.discuss);
                        updateSetting("vote_seconds", p.vote);
                      }}
          ...
          {settingsTab === "players" && (
            <div className="space-y-4">
              ...
              <SettingCard
                ...
                description="Game won't start below this"
                value={minPlayers}
                min={3}
                max={maxPlayers}
                step={1}
              ...
              <SettingCard
                ...
                description="Room closes above this"
                value={maxPlayers}
                min={minPlayers}
                max={16}
                step={1}
          ...
          {settingsTab === "timers" && (
            <div className="space-y-4">
              <TimerSetting
                icon="📝"
                label="Describe"
                description="Time to write a description"
              ...
              <TimerSetting
                icon="💬"
                label="Discuss"
                description="Time to read and discuss"
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <span
          className="flex-1 text-center text-xl font-display font-bold tabular-nums"
          style={{ color: "var(--text)" }}
        >
          {value}
        </span>
