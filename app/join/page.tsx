...
function JoinForm() {
  ...
  async function handleJoin() {
    ...
    try {
      ...
      if (!userRes.ok)
        throw new Error(userData.error || "Failed to save account");
      const res = await fetch(`/api/rooms/${fullCode}/join`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nickname: nickname.trim(), deviceToken }),
      });
      // In handleJoin(), after successful join:
      const data = await res.json();
      ...
      if (data.rejoining) {
        router.push(`/room/${fullCode}/game`);
      } else {
    ...
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-xs space-y-6 animate-fadeUp">
        ...
        <div className="text-center space-y-2">
          <div
            className="inline-flex items-center justify-center w-14 h-14 rounded-2xl text-2xl mb-1"
            style={{
